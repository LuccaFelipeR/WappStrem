...
import { TMBD_API_KEY, castImageUrl, castUrl, tmdbBase } from '../../../../../config/url';
import ReactPlayer from 'react-player';
import { CloseOutlined } from '@mui/icons-material';
// import MoreLikeThis from '../../pages/dashboard/preview/movies/more';
...
const MoviePreviewM3u = () => {
    ...
    const [yearNotDefined, setYearNotDefined] = useState(false);
    ...
    useEffect(() => {
        if (stream) {
            setStreamUrl(AES.decrypt(stream, 'thisisurl').toString(enc.Utf8));
        };
    }, [stream]);
    ...
    const removeBrackets = () => {
        // Regular expression pattern to match brackets and their contents
        const decrypted = AES.decrypt(decodeURIComponent(item), "thisismovie").toString(enc.Utf8);
        const index = decrypted.indexOf('('); // Find the index of the opening parenthesis

        if (index !== -1) {
            return decrypted.substring(0, index).trim(); // Extract substring before '(' and trim any spaces
        }

        return decrypted; // Return the original string if '(' is not found
    }

    function getContentInBrackets() {
        // Regular expression pattern to match brackets and their contents
        const pattern = /\(([^)]*)\)|\[([^\]]*)\]|\{([^}]*)\}/;
        const decrypted = AES.decrypt(decodeURIComponent(item), "thisismovie").toString(enc.Utf8);
        // Matched content within brackets
        let matchedContent = '';
        decrypted.replace(pattern, (match, round, square, curly) => {
            matchedContent = round || square || curly; // Get the content inside brackets
        });
    ...
    const getMovie = async () => {
        ...
        const endpoints = {
            getId: `/search/movie?api_key=${TMBD_API_KEY}&query=`,
            getMovie: `/movie`
        }

        const movieYear = getContentInBrackets();
        const decrypted = AES.decrypt(decodeURIComponent(item), "thisismovie").toString(enc.Utf8);

        try {
            const response = await axios.get(tmdbBase + endpoints.getId + removeBrackets());
            const currentYearMovie = response.data.results.filter(mov => String(new Date(mov.release_date).getFullYear()) === String(movieYear))[0];
            // console.log(response.data.results)
            if (currentYearMovie) {
                const tmbdbId = currentYearMovie.id;
                const movieInfo = await axios.get(tmdbBase + endpoints.getMovie + `/${tmbdbId}?api_key=${TMBD_API_KEY}`);
                const movie = movieInfo.data;
                setMovie(movie);
            } else {
                setYearNotDefined(true);
                setPlayerOpen(true)
                setMovie({
                    name: decrypted
                });
    ...
    const getMovieTimeline = async () => {
        const streamId = id;
        try {
            const response = await getParticluarTimeline(concatUrl(streamUrl), 'Movie', finalAddress);
            if (response.val()) {
    ...
    const getMovieCast = async () => {
        try {
            const response = await axios.get(castUrl + movie.id + '/credits?api_key=' + TMBD_API_KEY);
    ...
    const handleFavourites = async () => {
        ...
        if (addedToFav) {
            try {
                await removeFromFavs(concatUrl(streamUrl), "Movie", finalAddress);
                alert.toggle({
            ...
            try {
                await addToFavs(concatUrl(streamUrl), "Movie", finalAddress);
                alert.toggle({
    ...
    const checkFav = async (address) => {
        try {
            const response = await getValue(concatUrl(streamUrl), "Movie", address);
                    ...
                    <section className="mainBanner">
                        <div className="item">
                            ...
                            <div className="info">
                                ...
                                <p className="text"><b>Genre:</b> {movie?.genres.map((genre, index) => index !== 0 ? `/${genre.name}` : genre.name)}  <b>Directed By:</b> {movieCrew?.filter(cast => cast.job === 'Director')[0].name}</p>
                                    ...
                                    movieCast?.map((cast, index) => (
                                        <div key={index} className="item">
                                            <div className="thumb">
                                                {
                                                    cast.profile_path ?
                                                        <img src={cast.profile_path ? castImageUrl + cast.profile_path : "/placeholder.png"} /> :
...
export default MoviePreviewM3u