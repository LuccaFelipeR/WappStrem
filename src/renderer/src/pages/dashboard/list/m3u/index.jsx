...
import Scrollable from "../../../../helpers/scrollable/index"
import { Grid, List } from 'react-virtualized';
...
const M3uList = ({ finalAddress, currentAction }) => {
  ...
  // const { query } = router;
  // const { view } = query;
  const { m3uStreams, m3uUrl, homeM3uStreams, parentalVerified, scrolled } = useContext(AppContext);
  const refs = useRef([]);
  ...
  useEffect(() => {
    if (!movies && !series && m3uUrl.url) {
      getM3uStreams();
    };
  }, [m3uStreams, m3uUrl.url]);
  ...
  }, [finalAddress, currentAction, currentSelected]);
  ...
  useEffect(() => {
    if (movies || series || live) {
      ...
      if (isCategory) {
        movies.map(mov => setMovieCategories(prev => {
  ...
  const getFavs = async (values) => {
    ...
    try {
      ...
      if (ids) {
        let favs = []
        ids.map(id => {
          const itemExists = currentSelected.filter(movie => String(concatUrl(movie.url)) === String(id));
      ...
      if (response.val()) {
        ...
        ids.map((id, index) => {
          setRecents(prev => {
            ...
            const info = currentSelected.filter(movie => String(concatUrl(movie.url)) === String(id))[0];
            return [...updated, {
              id,
              ...data[index],
              info
  ...
  const FavsCategory = ({ favMovies, style }) => {
    const handleItem = (isAdult, movie) => {
      const encryptedName = encodeURIComponent(AES.encrypt(movie.name, 'thisismovie').toString());
      if (parentalPin) {
        if (isAdult) {
          ...
          router(`/dashboard/preview/${currentAction}/m3u/${encryptedName}?id=${getStreamIdFromUrl(movie.url)}&stream=${AES.encrypt(movie.url, 'thisisurl').toString()}`,
)
        }
      } else {
        router( `/dashboard/preview/${currentAction}/m3u/${encryptedName}?id=${getStreamIdFromUrl(movie.url)}&stream=${AES.encrypt(movie.url, 'thisisurl').toString()}`,
    ...
    return (favMovies && favMovies.length > 0) && <section
      ...
      <Scrollable>
        {favMovies?.map((movie, index) => {
          ...
          return <div key={index} onClick={() => handleItem(isAdult, movie)} style={{ marginTop: 10, marginBottom: 10, width: 165, borderRadius: 5 }} className="item">
  ...
  const Recents = ({ style }) => {
    ...
    const handleItem = (isAdult, movie) => {
      const encryptedName = encodeURIComponent(AES.encrypt(movie.info.tvg.name, 'thisismovie').toString());
      ...
      if (parentalPin) {
        if (isAdult) {
          ...
          setShowParentalLock(false);
          router( `/dashboard/preview/${currentAction}/m3u/${encryptedName}?state=play&id=${getStreamIdFromUrl(movie.id)}&stream=${AES.encrypt(movie.info.url, 'thisisurl').toString()}`,
)
        }
      } else {
        router( `/dashboard/preview/${currentAction}/m3u/${encryptedName}?state=play&id=${getStreamIdFromUrl(movie.id)}&stream=${AES.encrypt(movie.info.url, 'thisisurl').toString()}`,
    ...
    return recents.length > 0 && <section style={style} className="category">
      ...
      <Scrollable>
        {recents.map((movie, index) => {
          ...
          const isFavourite = FavouriteMovies ? FavouriteMovies.filter(item => String(concatUrl(item.url)) === String(concatUrl(movie.id))).length > 0 : null;
          const isAdult = movie?.info && adultArray.filter(item => movie.info.group.title.toLowerCase().includes(item.toLowerCase())).length > 0;
          return <div key={index} onClick={() => handleItem(isAdult, movie)} style={{ marginTop: 10, marginBottom: 10, width: 165, }} className="item">
            <div className="caption">
              <span className="control">
  ...
  const MovieCategory = ({ index, style }) => {
    const filtered = movies.filter(movie =>
      (movie.group.title === movieCategories[index - moviesDifference]))
      ;
    ...
    const innerElement = (args) => {
      ...
      const handleItem = () => {
        scrolled.toggle(0, index, 0)
        const encryptedName = encodeURIComponent(AES.encrypt(filteredComp.name, 'thisismovie').toString())
        ...
        if (parentalPin) {
          if (isAdult) {
            ...
            setShowParentalLock(false);
            router(`/dashboard/preview/${currentAction}/m3u/${encryptedName}?id=${getStreamIdFromUrl(filteredComp.url)}&stream=${AES.encrypt(filteredComp.url, 'thisisurl').toString()}&name=${filteredComp.url}`,
);
          };
        } else {
          router(`/dashboard/preview/${currentAction}/m3u/${encryptedName}?id=${getStreamIdFromUrl(filteredComp.url)}&stream=${AES.encrypt(filteredComp.url, 'thisisurl').toString()}`,
);
        }
      };
            ...
            <div className="thumb">
              {
                ...
                <img
                  ...
                  src={
                    movie_image ? movie_image : '/placeholder.png'
                  }
    ...
    return filtered.length > 0 && <section key={index} style={style} className="category listSlider">
      <span className="h3">{movieCategories[index - moviesDifference]}</span>
      <div className="list"
      >
        <div style={{
          paddingLeft: 10
  ...
  const SeriesCategory = ({ index, style }) => {
    ...
    return filtered.length > 0 && <section key={index} style={style} className="category listSlider">
      <span className="h3">{seriesCategories[index - seriesDifference]}</span>
      <div className="list">
        <div style={{
          paddingLeft: 10
        }} className="owl-carousel owl-theme"
  ...
  const renderMovie = ({ index, key, style }) => {
    ...
    return <MovieCategory key={key} style={style} index={index} />
  };

  const renderSeries = ({ index, key, style }) => {
    if (favouriteSeries && favouriteSeries.length > 0 && index === 0) {
      setSeriesDifference(1)
    }

    if (favouriteSeries && favouriteSeries.length > 0 && index === 0) {

      return <FavsCategory key={key} style={style} favMovies={favouriteSeries} />
    };

    return <SeriesCategory key={key} style={style} index={index} />
  };
    ...
    <div className='m3u-list-container' style={{
      ...
      <ParentalLock
        ...
        completed={() => {
          ...
          const encryptedName = encodeURIComponent(AES.encrypt(clickedAdultItem.name, 'thisismovie').toString());

          router(`/dashboard/preview/${currentAction}/m3u/${encryptedName}?id=${getStreamIdFromUrl(filteredComp.url)}&stream=${AES.encrypt(clickedAdultItem.url, 'thisisurl').toString()}`,
);
        }}
      />
      {
        loading ?
          <Loading />
          :
          ((currentSelected && currentSelected.length === 0) || (currentAction === 'movies' && movieCategories.length === 0) || (currentAction === 'series' && seriesCategories.length === 0)) ?
            <div className="no-data-found-container">
              <img src={noContentFound} />
            ...
            <>
              {(view === 'movies' && movies && (movies.length > 0)) ?
                ...
                />
                :
                view === 'series' && series && (series.length > 0) &&
                <List
                  width={windowSize.width - 10}
                  ...
                  rowCount={seriesCategories?.length + (favouriteSeries && favouriteSeries.length > 0 ? 0 : 0)}
                  overscanRowCount={3}
                />
              }
            </>
